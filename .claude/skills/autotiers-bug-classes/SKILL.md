...
- **Verify cloud preconditions before building infra that assumes them — don't trust a stated answer.** `infra/ses.tf` was first written (#245) to publish DNS via `data.aws_route53_zone` because the operator said the zone was in Route 53; it was not (the account had zero hosted zones), so `terraform plan` failed — a `data.aws_route53_zone` lookup resolves at plan time and errors when the zone is absent — and the module had to be reworked for external DNS (#248). A one-line read-only check (`aws route53 list-hosted-zones`, `aws acm list-certificates`, etc.) confirms a precondition before you couple a module to it. The same applies to "the cert exists", "the bucket exists", "the VPC has these subnets".
- **With local Terraform state (no remote backend), the state can lag merged config — review the FULL plan for unrelated drift before applying, and isolate intended changes with `-target`.** Preparing the SES apply surfaced a pending, unrelated ALB security-group *replacement* (an HTTPS rollout merged to `main` but never applied — #249) that a blind `apply` would have dragged into a prod change. Never apply only the summary line; read every `-/+ must be replaced` and `# ... will be destroyed` against what you intended. Reach for `-target` sparingly — Terraform itself warns it produces partial applies and can leave further drift behind, so treat it as a recovery/scoping lever (as in #248), not a routine workflow, and follow up with a full plan/apply to reconcile once the unrelated change ships (#251).

## 10. Duplicate dependency instances → stranded global state

**Canonical case:** the **Link Accounts** modal closed via outside-click left the entire page inert — every click dead (#279). Two module instances of `@radix-ui/react-dismissable-layer` coexisted in `web/node_modules`: `v1.1.11` at the root (pulled by `@radix-ui/react-menu`, the hamburger dropdown) and `v1.1.12` nested under `@radix-ui/react-dialog/node_modules/` (the modal). Each copy has its OWN module-level `DismissableLayerContext` and its own `originalBodyPointerEvents` save variable. The menu sets `body.style.pointerEvents = "none"`; on dialog outside-click close the cleanup ran against the *wrong* instance's context, so `body` stayed stuck at `"none"`. `@radix-ui/react-focus-guards` had the same split (1.1.3 vs 1.1.4, a count-underflow bug). The `v1.1.11` copy had no `resolved` field in the lockfile — it had been hand-placed, not installed by npm (a prior failed manual-vendor attempt, leaving two orphaned `.tgz` files at the repo root). The fix pinned both packages via `overrides` in `web/package.json` so all consumers share a single instance.

**The general rule:** any library that keeps state in a **module-level singleton** (a context object, a counter, a `Set` of active layers, a saved original style) is only correct if exactly ONE copy is loaded. Two copies = two independent singletons that each think they own the global resource; whichever cleans up last (or first, against the wrong instance) corrupts the shared DOM/body state. Focus traps, scroll locks, portal/overlay managers, and `pointer-events`/`inert`/`aria-hidden` body locks are the usual victims because their symptom is the *whole page* going dead, not a localized error.

**How to detect:**
- After any dependency bump, lockfile regen, or new Radix/overlay component, run `npm ls <pkg>` for the stateful primitive and confirm a SINGLE resolved version. Also `find web/node_modules -path '*/node_modules/@radix-ui/<pkg>'` — any nested copy means duplication. All consumers should read "deduped"/"overridden".
- A lockfile entry with **no `resolved` field** is a red flag: it was hand-placed, not installed. So are stray `*.tgz` files at the repo root — they are failed manual-vendor attempts, not a fix. Delete them and pin via `overrides` instead.
