...
variable "domain_name" {
  description = <<-EOT
    ...
    Route 53 hosted zone these records live in (var.route53_zone_name).
  EOT
  type        = string
...
###############################################################################
# DNS — apex/www/api Route 53 records (see dns.tf)
###############################################################################
variable "route53_zone_name" {
  description = "Name of the Route 53 hosted zone that owns domain_name (only used when manage_dns = true). The apex/www/api records (dns.tf) are published here."
  type        = string
  default     = "auto-tiers.com"
}

variable "manage_dns" {
  description = "When true, Terraform publishes the apex/www/api records (dns.tf) to Route 53 (zone must exist in this account). When false (default), DNS is managed externally and Terraform does not touch it."
  type        = bool
  default     = false
}
