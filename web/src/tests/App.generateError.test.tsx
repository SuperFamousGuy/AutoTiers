...
describe("App — generate failure is surfaced, not swallowed (#607)", () => {
  it("shows a role=alert error affordance and a toast on a 500, not the empty state", async () => {
    ...
    // …the toast fired from onError. Radix Toast renders its text twice — the
    // visible title AND a screen-reader announce region — so match all nodes
    // rather than assuming exactly one (getByText throws on multiple matches).
